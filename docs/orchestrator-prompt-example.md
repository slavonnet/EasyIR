...
Ниже шаблон промпта для запуска оркестратора. Он учитывает правила из `AGENTS.md` и `docs/roadmap.multi-agent.yaml`.
...
- Источник правды по задачам и зависимостям: docs/roadmap.multi-agent.yaml
...
3) Перед запуском сабагентов:
   - для каждой выбранной задачи выставь status: "В Работе" в docs/roadmap.multi-agent.yaml;
...
Используйте правило: задача считается **ready**, если для нее в `workstreams` все `depends_on` уже закрыты (или список пуст).
...
> Важно: перед запуском каждой волны сверяйте фактические `status` и `depends_on` в `docs/roadmap.multi-agent.yaml`, а также текущее состояние merged PR в `dev`.
